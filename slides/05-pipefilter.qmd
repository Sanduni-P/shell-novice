...
- Start in `shell-lesson-data/exercise-data/alkanes` (six molecule files):
...
- If the file doesn't exist, the shell will create the file. 
...
- `cat` (concatenate) prints file contents.
...
1. First 3 lines of `animals.csv`
2. Last 2 lines of `animals.csv`
3. First 3 lines AND last 2 lines of `animals.csv`
4. Second and third lines of `animals.csv`
...
- Add another pipe to `head`; the full pipeline becomes:
...
The `cut` command is used to select or 'cut out' certain sections of each line in the file for further processing while leaving the original file unchanged. By default, `cut` expects the lines to be separated into columns by a <kbd>Tab</kbd> character. A character used in this way is called a **delimiter**.  

In the example above we use the `-d` option to specify the comma as our delimiter character instead of <kbd>Tab</kbd>. We have also used the `-f` option to specify that we want to extract the second field (column). This gives the following output:
...
- Nelle ran samples and produced 17 files in `north-pacific-gyre`.
- Starting from `shell-lesson-data`, she runs a quick line-count check:
...
- Output has 18 lines, for example:
...
- One file is 60 lines shorter than the others.

## Checking the Short File

- She checks the run log and finds the assay was done Monday at 8:00.
- Likely cause: the machine was used over the weekend and not reset.

- Before re-running, she checks for files with too much data:
...
- Line counts look fine, but a `'Z'` appears in one filename.
- In her lab, sample IDs should be `A` or `B`; `Z` indicates missing metadata.
...
- Her log confirms both `Z` samples are missing depth values.
- She must exclude them from the current analysis.
- She keeps the files (instead of deleting) for possible later analyses.
- For now, she selects only `NENE*A.txt` and `NENE*B.txt`.
