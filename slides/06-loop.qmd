...
- They improve productivity through automation.
- Like wildcards and tab completion, loops reduce typing and mistakes.
...
- Imagine hundreds of genome files like `basilisk.dat`, `minotaur.dat`, and `unicorn.dat`.
- We use `exercise-data/creatures` (three files) for a small demonstration.
- The same approach scales to many files.
- Each file has common name, classification, and update date at the top.
...
- Goal: print the classification (line 2) for each species.
- Per file, use `head -n 2` piped to `tail -n 1`.

## General Form of a Loop

- We’ll solve this with a loop.
- First, look at the general loop form:
...
- Since `red dragon.dat` and `purple unicorn.dat` do not exist, running without quotes around `$filename` gives errors:
...
- The diagram shows loop execution flow.
- It also shows why using `echo` is useful for dry-run debugging.
...
- Nelle is ready to process files with `goostats.sh`.
- The script calculates statistics from a protein sample file.
- It takes two arguments:

1. an input file (containing the raw data)
2. an output file (to store the calculated statistics)

- Since she is still learning, she builds the commands in stages.

## Nelle Tests File Selection

- First step: confirm she selects only `A` and `B` files (not `Z`).
...
- Result: correct files are selected.

## Nelle Builds Output Filenames

- Next step: decide output filenames for `goostats.sh`.
- She prefixes each input file with `stats-`.
...
- Now she can generate correct output names.
- She has not run `goostats.sh` yet; this is a dry run.
